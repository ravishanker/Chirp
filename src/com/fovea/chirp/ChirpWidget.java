...
public class ChirpWidget extends AppWidgetProvider {
	private static final String TAG = ChirpWidget.class.getSimpleName();
		...
		if (intent.getAction().equals(UpdaterService.NEW_STATUS_INTENT)) {
			...
			this.onUpdate(context, appWidgetManager, 
					appWidgetManager.getAppWidgetIds(
							new ComponentName(context, ChirpWidget.class)));
		}
	}
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		try {
			if (c.moveToFirst()) {
				CharSequence user = c.getString(c.getColumnIndex(
						StatusData.C_USER));
				
				CharSequence createdAt = DateUtils.getRelativeTimeSpanString(
						context, c.getLong(c.getColumnIndex(
								StatusData.C_CREATED_AT)));
				
				CharSequence message = c.getString(
						c.getColumnIndex(StatusData.C_TEXT));
				
				for (int appWidgetId : appWidgetIds) {
					Log.d(TAG, "Updating widget " + appWidgetId);
					
					RemoteViews views = new RemoteViews(context.getPackageName(),
							R.layout.chirp_widget);
